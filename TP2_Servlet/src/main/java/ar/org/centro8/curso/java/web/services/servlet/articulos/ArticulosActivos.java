 ...
 * @author Ralu
 */
@WebServlet(name = "ArticulosActivos", urlPatterns = {"/ArticulosActivos"})
public class ArticulosActivos extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        response.setContentType("application/json;charset=UTF-8");
        I_ArticuloRepository ar = new ArticuloRepository(Connector.getConnection());
        try (PrintWriter out = response.getWriter()) {
            out.print(new Gson().toJson(ar.getByActivo(true)));
        }
    }
